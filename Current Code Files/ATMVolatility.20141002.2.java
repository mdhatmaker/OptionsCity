		...
		if ((updateCount % RISK_UPDATE_COUNT) == 1) {
			...
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				...
				for (FuturesContract futuresContract : product.getUnderlyingContracts()) {
					...
					DEBUG_MSG("$$$ " + futuresContract.getInstrumentId());					
					String rowSymbol = futuresContract.getInstrumentId();
					riskGrid.set(rowSymbol, "Delta", round(futuresContract.getDelta()));
					//riskGrid.set(rowSymbol, "Gamma", round(risk.gamma));
					//riskGrid.set(rowSymbol, "Theta", round(risk.theta));
					//riskGrid.set(rowSymbol, "Vega", round(risk.vega));
					riskGrid.set(rowSymbol, "DayTradeProfitLoss", round(futuresContract.getDayTradePandL()));
					riskGrid.set(rowSymbol, "ProfitLoss", round(futuresContract.getTotalPandL()));					
