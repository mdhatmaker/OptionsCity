...
public class ATMVolatility extends AbstractJob {
	...
	class RiskValues {
		...
		public double vega = 0.0;
		public double dayTradeProfitAndLoss = 0.0;
		public double profitAndLoss = 0.0;

		public RiskValues() {

		}

		public RiskValues(ConsolidatedRisk risk) {
			delta = risk.delta;
			gamma = risk.gamma;
			theta = risk.theta;
			vega = risk.vega;
			dayTradeProfitAndLoss = risk.dayTradeProfitAndLoss;
			profitAndLoss = risk.getTotalPandL();
		}

		public void add(ConsolidatedRisk risk) {
			delta += risk.delta;
			gamma += risk.gamma;
			theta += risk.theta;
			vega += risk.vega;
		...
		public void add(FuturesContract futuresContract) {
			...
			profitAndLoss += futuresContract.getTotalPandL();			// adjust total P&L by the futures total P&L			
		}
	}

	void displayRiskValuesInGrid(RiskValues riskValues, IGrid grid, String rowSymbol) {
		grid.set(rowSymbol, "Delta", round(riskValues.delta));
		grid.set(rowSymbol, "Gamma", round(riskValues.gamma));
		grid.set(rowSymbol, "Theta", round(riskValues.theta));
		grid.set(rowSymbol, "Vega", round(riskValues.vega));
		grid.set(rowSymbol, "DayTradeProfitLoss", round(riskValues.dayTradeProfitAndLoss));
		grid.set(rowSymbol, "ProfitLoss", round(riskValues.profitAndLoss));		
	}
	...
	IGrid riskGrid;
	IGrid riskSummaryGrid;
	IGrid riskTotalsGrid;
	...
	public void begin(IContainer container) {		
		...
		*/
		...
		riskSummaryGrid = container.addGrid("RiskSummary", new String[] {"Delta", "Gamma", "Theta", "Vega", "Downside", "ATM", "Upside", "DayTradeProfitLoss", "ProfitLoss", "OptionsDelta", "FuturesDelta", "ExpirationDate"});
		riskSummaryGrid.clear();
		log("To use, create a grid named \"RiskSummary\" with columns \"Delta\", \"Gamma\", \"Theta\", \"Vega\", \"Downside\", \"ATM\", \"Upside\", \"DayTradeProfitLoss\", \"ProfitLoss\", \"OptionsDelta\", \"FuturesDelta\", and \"ExpirationDate\"");

		riskTotalsGrid = container.addGrid("RiskTotals", new String[] {"Delta", "Gamma", "Theta", "Vega", "Downside", "ATM", "Upside", "DayTradeProfitLoss", "ProfitLoss", "OptionsDelta", "FuturesDelta", "ExpirationDate"});
		riskTotalsGrid.clear();
		log("To use, create a grid named \"RiskTotals\" with columns \"Delta\", \"Gamma\", \"Theta\", \"Vega\", \"Downside\", \"ATM\", \"Upside\", \"DayTradeProfitLoss\", \"ProfitLoss\", \"OptionsDelta\", \"FuturesDelta\", and \"ExpirationDate\"");
	...
	public void onTimer() {
		...
		*/
			...
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				...
				for (FuturesContract futuresContract : product.getUnderlyingContracts()) {
					...
					for (OptionMonth month : monthsWithUnderlying) {
						rowSymbol = month.getDisplayText();
						sortChars = getSortChars(sortCharAsciiCode);
						riskGridRowSymbolTable.put(rowSymbol, sortChars + "     " + rowSymbol);
					}

					// THEN display the futures contract
					rowSymbol = futuresContract.getDisplayText();
					sortChars = getSortChars(sortCharAsciiCode++);
					riskGridRowSymbolTable.put(rowSymbol, sortChars + rowSymbol);
				}

				// FINALLY display the product symbol by itself (this will serve as the summary row for all months in a product)
				rowSymbol = product.getDisplayText();
				sortChars = getSortChars(sortCharAsciiCode++);
				riskGridRowSymbolTable.put(rowSymbol, sortChars + "     *** " + rowSymbol + " ***");
			}			
		}
		...
		if ((updateCount % RISK_UPDATE_COUNT) == 1) {
			RiskValues totalRisk = new RiskValues();

			log("***riskUpdate");
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				RiskValues productRisk = new RiskValues();

				Product product = entry.getValue();
				product.updatePositionRiskForAllMonths();
				for (FuturesContract futuresContract : product.getUnderlyingContracts()) {
					
					RiskValues contractRisk = new RiskValues();
					...
					for (OptionMonth month : monthsWithUnderlying) {
						...
						RiskValues optionMonthRisk = new RiskValues(risk);

						String rowSymbol = riskGridRowSymbolTable.get(month.getDisplayText());
						displayRiskValuesInGrid(contractRisk, riskGrid, rowSymbol);

						contractRisk.add(risk);
						productRisk.add(risk);
						totalRisk.add(risk);
					...
					contractRisk.add(futuresContract);
					productRisk.add(futuresContract);
					totalRisk.add(futuresContract);

					// THEN display the futures contract
					DEBUG_MSG("$$$ " + futuresContract.getDisplayText());					
					String rowSymbol = riskGridRowSymbolTable.get(futuresContract.getDisplayText());
					displayRiskValuesInGrid(contractRisk, riskGrid, rowSymbol);
					displayRiskValuesInGrid(contractRisk, riskSummaryGrid, rowSymbol);

					/*// Sum the greeks for this futures contract
					contractRisk.delta += futuresContract.getDelta();	// adjust the delta by the futures delta
					contractRisk.dayTradeProfitAndLoss += futuresContract.getDayTradePandL();	// adjust daytrade P&L by the futures daytrade P&L
					contractRisk.profitAndLoss += futuresContract.getTotalPandL();			// adjust total P&L by the futures total P&L

					// We also need to sum the greeks for ALL MONTHS AND FUTURES of the PRODUCT
					productRisk.delta += futuresContract.getDelta();
					productRisk.dayTradeProfitAndLoss += futuresContract.getDayTradePandL();
					productRisk.profitAndLoss += futuresContract.getTotalPandL();*/

					/*displayValuesInGrid(contractRisk, riskGrid, rowSymbol);
				...
				displayRiskValuesInGrid(productRisk, riskGrid, rowSymbol);
				displayRiskValuesInGrid(productRisk, riskSummaryGrid, rowSymbol);
				displayRiskValuesInGrid(productRisk, riskTotalsGrid, rowSymbol);
				...
				/*riskSummaryGrid.set(rowSymbol, "Delta", round(productDelta));
				riskSummaryGrid.set(rowSymbol, "Gamma", round(productGamma));
				riskSummaryGrid.set(rowSymbol, "Theta", round(productTheta));
				riskSummaryGrid.set(rowSymbol, "Vega", round(productVega));
				riskSummaryGrid.set(rowSymbol, "DayTradeProfitLoss", round(productDayTradeProfitAndLoss));
				riskSummaryGrid.set(rowSymbol, "ProfitLoss", round(productTotalProfitAndLoss));*/
			...
			// Display the TOTAL row for each of the grids
			String rowSymbol = riskGridRowSymbolTable.get("TOTAL");
			displayRiskValuesInGrid(totalRisk, riskGrid, rowSymbol);
			displayRiskValuesInGrid(totalRisk, riskSummaryGrid, rowSymbol);
			displayRiskValuesInGrid(totalRisk, riskTotalsGrid, rowSymbol);
