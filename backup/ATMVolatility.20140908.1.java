...
public class ATMVolatility extends AbstractJob {
	...
	public void updateGreeks(String id) { //throws NullPointerException {
		...
		for (Map.Entry<String, Product> entry : products.entrySet()) {
			...
			for (Map.Entry<String, OptionMonth> monthEntry : product.getMonths().entrySet()) {
				...
				for (String id : optionMonth.getIds()) {
					//log("Update greeks for instrumentId: " + id); n 
					updateGreeks(id);
					/*try {
						updateGreeks(id);
					}
					catch (NullPointerException ex) {
						log("EXCEPTION updating greeks for " + id + ": " + ex.getMessage());
					}*/
				}
			}
		}
