...
public class ATMVolatility extends AbstractJob {
	...
	class OptionMonth {
		...
		public void calculateAverageATMVolatilities(int count) {
			List<OptionStrike> atmStrikes = getATMStrikes(count);
			//log("ATM strike count: " + String.valueOf(atmStrikes.size()));

			double averageVol = 0.0;
			double averageBidVol = 0.0;
			double averageAskVol = 0.0;
			for (OptionStrike atmStrike : atmStrikes) {
	...
	public void onTimer() {
		...
		*/
		...
		for (Map.Entry<String, Product> entry : products.entrySet()) {
			...
			log("Product Symbol (publish ATM vols): " + symbol + "  [" + String.valueOf(monthCount) + " months]");
			...
			if (monthCount > 0) {
				...
				for (int i=0; i<atmVolMonthCount; ++i) {
					...
					log("PROBE (" + symbol + idx + ")  bidvol: " + String.valueOf(bidVol) + "   vol: " + String.valueOf(vol) + "   askvol: " + String.valueOf(askVol));
		...
		if ((updateCount % VOL_CHART_UPDATE_COUNT) == 1) {
			...
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				Product product = entry.getValue();
				int monthCount = product.monthCount();
				log("Product Symbol (model and implied vols): " + product.getSymbol() + "  [" + String.valueOf(monthCount) + " months]");
