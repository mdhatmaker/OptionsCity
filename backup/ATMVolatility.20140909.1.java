...
public class ATMVolatility extends AbstractJob {
	...
	class OptionStrike {
		...
		private void updateImpliedVols() {
			String id = call.getId();
			Prices price = instrumentService.getAllPrices(id);
			double midPrice = (price.underlying_bid + price.underlying_ask) / 2.0;
			double bidVol = theoService.calculateImpliedVolatility(id, price.bid, price.underlying_bid);
			//double bidVol = theoService.calculateImpliedVolatility(id, price.theo_bid, midPrice);
			//double bidVol = theoService.calculateImpliedVolatility(id, price.theo_bid, price.underlying_bid);
			double askVol = theoService.calculateImpliedVolatility(id, price.ask, price.underlying_ask);
			//double askVol = theoService.calculateImpliedVolatility(id, price.theo_ask, midPrice);
			//double askVol = theoService.calculateImpliedVolatility(id, price.theo_ask, price.underlying_ask);
			bidImpliedVol = bidVol * 100.0;
			bidImpliedVol = round(bidImpliedVol);			
			askImpliedVol = askVol * 100.0;
			askImpliedVol = round(askImpliedVol);			
		}
	...
	class Product {
		...
		public void setBidGreeks(String id, Greeks greeks) {
			getStrike(id).setBidGreeks(greeks);
		}

		public void setAskGreeks(String id, Greeks greeks) {
			getStrike(id).setAskGreeks(greeks);
		}
	...
	public void begin(IContainer container) {		
		...
		*/
		...
		frontMonthFuturesGrid = container.addGrid("FrontMonthFutures", new String[] {"Position", "BidQty", "Bid", "Ask", "AskQty", "atmBidVol", "atmAskVol", "Delta", "Gamma", "Theta"});
		frontMonthFuturesGrid.clear();
		log("To use, create a grid named \"FrontMonthFutures\" with columns \"Position\", \"BidQty\", \"Bid\", \"Ask\", \"AskQty\", \"atmBidVol\", \"atmAskVol\", \"Delta\", \"Gamma\", and \"Theta\"");
	...
	public void onTimer() {
		...
		// ****************************************
		// * Next, iterate to retrieve the ATM vols
		// ****************************************
		for (Map.Entry<String, Product> entry : products.entrySet()) {
			String symbol = entry.getKey().toLowerCase();
			log("Product Symbol (get ATM vols): " + symbol);
			Product product = entry.getValue();
		...
		if ((updateCount % VOL_CHART_UPDATE_COUNT) == 1) {
			...
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				Product product = entry.getValue();
				log("***Retrieve model volatilities: " + product.getSymbol() + "***");
				...
				if (month != null) {
					...
					for (String strikeId : month.getStrikeKeys()) {
						...
						Greeks bidGreeks = optionStrike.getBidGreeks();
						Greeks askGreeks = optionStrike.getAskGreeks();
						String symbol = product.getSymbol();
						//log(strikeId + "   bidtheo:" + roundStr(bidGreeks.theo) + " asktheo:" + roundStr(askGreeks.theo) + "  bidimpl:" + roundStr(bidGreeks.volatility * 100.0) + " askimpl:" + roundStr(askGreeks.volatility * 100.0) + "  modelvol:" + roundStr(modelVolatility));
						log(symbol + " " + strikeId + "   bidtheo:" + roundStr(bidGreeks.theo) + " asktheo:" + roundStr(askGreeks.theo) + "  bidimpl:" + roundStr(bidImplied) + " askimpl:" + roundStr(askImplied) + "  modelvol:" + roundStr(modelVolatility));
