...
public class ATMVolatility extends AbstractJob {
	...
	class Option {
		...
		public void calculateModelVolatility() {
			Prices price = instrumentService.getAllPrices(instrumentId);
			double bidVol = theoService.calculateImpliedVolatility(instrumentId, price.theo_bid, price.underlying_bid);
			double askVol = theoService.calculateImpliedVolatility(instrumentId, price.theo_ask, price.underlying_ask);
			modelVolatility = (bidVol + askVol) / 2 * 100.0;
			modelVolatility = Math.round(modelVolatility * 100.0) / 100.0;
			log(instrumentId + " " + String.valueOf(modelVolatility));
		}
	}
	...
	class OptionStrike {
		...
		public void calculateModelVols() {
			call.calculateModelVolatility();
			put.calculateModelVolatility();		
		}
	}
	...
	class Product {
		...
		public OptionMonth getMonth(String expirationMonth) {
			return months.get(expirationMonth);
		}
		...
		public void calculateModelVolsForMonth(String expirationDate) {
			months.get(expirationDate).calculateModelVols();
		}
	...
	public void addProduct(String productSymbol) {
		...
		/*volGrids.put("OZS", container.addGrid("OZS", new String[] {"Strike", "Vol", "Implied"}));
		volGrids.get("OZS").clear();*/

    	/*products.put("LE", new Product("LE"));
    	products.put("HE", new Product("HE"));
    	products.put("OZS", new Product("OZS"));
    	products.put("OZW", new Product("OZW"));
    	products.put("OZM", new Product("OZM"));
    	products.put("OZL", new Product("OZL"));*/		
	}
	...
	public void onTimer() {
		...
		if ((updateCount % VOL_CHART_UPDATE_COUNT) == 1) {
			...
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				...
				OptionMonth month = product.getMonth("201409");
				if (month != null) {
					...
					for (String strikeId : strikes.keySet()) {
						...
						Option callOption = optionStrike.getCall();
						//Option putOption = strikes.get(strikeId).getPut();
						log("strikeId:" + strikeId + "   *** strike:" + String.valueOf(optionStrike.getStrikeValue()) + "  callID:" + callOption.getId());
						volGrids.get(product.getSymbol()).set(strikeId, "Strike", optionStrike.getStrikeValue());
						volGrids.get(product.getSymbol()).set(strikeId, "Vol", callOption.getModelVolatility());
					}
				}
				else {
					log("month is NULL");
