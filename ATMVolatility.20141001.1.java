...
public class ATMVolatility extends AbstractJob {
	...
	class Option {
		...
		public void updatePositionRisk() {
			positionRisk = positionService.getInstrumentRisk(instrumentId);
		}
	...
	class OptionMonth {
		...
		public OptionMonth(String productSymbol, String expirationDate, InstrumentDetails details) {
			...
			underlying = futuresContracts.get(details.underlyingId);
		...
		public void updatePositionRiskForAllStrikes() {
			for (Map.Entry<String, OptionStrike> entry : strikes.entrySet()) {
				OptionStrike strike = entry.getValue();
				strike.getCall().updatePositionRisk();
				strike.getPut().updatePositionRisk();
			}	
		}
		...
		public Risk getConsolidatedRisk() {
			Risk result = new Risk();
			for (Map.Entry<String, OptionStrike> entry : strikes.entrySet()) {
				OptionStrike strike = entry.getValue();
				//result.add(strike.call.getGreeks());
				//result.add(strike.put.getGreeks());
				result.add(strike.call.getPositionRisk());
				result.add(strike.put.getPositionRisk());
			}
			return result;
		}
	...
	public void begin(IContainer container) {		
		...
		*/
		...
		// Must discover the futures (FuturesContract) that will serve as the underlyings before we add options with the addOption() method
		...
		riskGrid = container.addGrid("Risk", new String[] {"Delta", "Gamma", "Theta", "Vega", "Downside", "ATM", "Upside", "ProfitLoss", "OptionsDelta", "FuturesDelta", "ExpirationDate"});
		riskGrid.clear();
		log("To use, create a grid named \"Risk\" with columns \"Delta\", \"Gamma\", \"Theta\", \"Vega\", \"Downside\", \"ATM\", \"Upside\", \"ProfitLoss\", \"OptionsDelta\", \"FuturesDelta\", and \"ExpirationDate\"");
		...
		if ((updateCount % RISK_UPDATE_COUNT) == 1) {
			...
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				...
				for (Map.Entry<String, OptionMonth> monthEntry : product.getMonths().entrySet()) {
					...
					Risk risk = month.getConsolidatedRisk();
