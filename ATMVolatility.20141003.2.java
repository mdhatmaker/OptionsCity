	...
	public void onTimer() {
		...
		*/
		...
		if (riskGridRowSymbolTable == null) {
			...
			int sortCharAsciiCode = 128;	// starting ASCII char to use for sorting rows alphabetically
			...
			for (Map.Entry<String, Product> entry : products.entrySet()) {
				...
				for (FuturesContract futuresContract : product.getUnderlyingContracts()) {
					...
					for (OptionMonth month : monthsWithUnderlying) {
						...
						String sortChar = new Character((char) sortCharAsciiCode++).toString();
						riskGridRowSymbolTable.put(rowSymbol, sortChar + "     " + rowSymbol);
					}
